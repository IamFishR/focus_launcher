...
import 'package:focus_launcher/settings_page.dart'; // Correct import path
...
void main() {
  ...
  const List<Map<String, dynamic>> mockApps = [
    {
      ...
      'icon': null
    }, // Assuming icon can be null or byte data
  ...
  group('App Visibility Settings', () {
    testWidgets('shows loading indicator then app list',
      ...
      expect(find.byType(CircularProgressIndicator),
          findsWidgets); // Might be more than one if fav apps also load
      ...
      await tester
          .pumpAndSettle(); // Let SharedPreferences and app loading complete
    ...
    testWidgets('all apps visible by default (switches ON)',
        (WidgetTester tester) async {
      SharedPreferences.setMockInitialValues({
      ...
      await tester.pumpWidget(const MaterialApp(home: SettingsPage()));
      await tester
          .pumpAndSettle(); // Wait for async operations like loading apps and prefs

      // Verify all SwitchListTile widgets are value == true
      for (var app in mockApps) {
        final appName = app['name'] as String;
        final switchTile = tester.widget<SwitchListTile>(
            find.widgetWithText(SwitchListTile, appName));
        expect(switchTile.value, isTrue,
            reason: '$appName switch should be ON');
      }
    });

    testWidgets('loads hidden apps correctly (switches OFF for hidden)',
        (WidgetTester tester) async {
      SharedPreferences.setMockInitialValues({
      ...
      final switchTileB = tester
          .widget<SwitchListTile>(find.widgetWithText(SwitchListTile, 'App B'));
      ...
      final switchTileA = tester
          .widget<SwitchListTile>(find.widgetWithText(SwitchListTile, 'App A'));
      expect(switchTileA.value, isTrue, reason: 'App A switch should be ON');
      final switchTileGemini = tester.widget<SwitchListTile>(
          find.widgetWithText(SwitchListTile, 'Gemini'));
      expect(switchTileGemini.value, isTrue,
          reason: 'Gemini switch should be ON');
    });

    testWidgets('toggling switch updates SharedPreferences and UI',
        (WidgetTester tester) async {
      ...
      expect(prefsAfterUnhide.getStringList(hiddenAppPackagesKey),
          isNot(contains('com.appa')));
